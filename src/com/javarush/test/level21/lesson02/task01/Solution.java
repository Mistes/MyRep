...
public class Solution {
    ...
    public static byte[] getNetAddress(byte[] ip, byte[] mask) {
        byte[] cute = new byte[ip.length];
        for (int i = 0; i<cute.length; i++){
            cute[i] = (byte)(ip[i]&mask[i]);
        }
        return cute;
    }
    ...
    public static void print(byte[] bytes) {
        ...
        for(int i=0;i<bytes.length;i++) {
            if (i == 3) System.out.format("%08d", Integer.parseInt(Long.toBinaryString(bytes[i])));
            else System.out.format("%08d ", Integer.parseInt(Long.toBinaryString(bytes[i])));
        }
        System.out.println();
       }
